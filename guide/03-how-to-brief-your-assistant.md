...
Between the two approvals, let Bob work. Approving every file in Agent mode removes the benefit of the mode. Control in Agent mode comes from the checks described in 3.4.

## 3.3 The types of prompts

A prompt is the text that you type in the chat. This guide uses five kinds of prompt, one for each situation.
...
A question asks Bob to find information and report it, without changing anything. Examples: "Which agents exist on my instance?", "Why did the last test answer that the order was unknown?"
...
Limitations: a question does not create or change anything. The answer is based on what Bob has read. Name the file or the test that Bob must examine; otherwise, Bob answers from its general knowledge.
...
A brief describes what you want in your own words. The watsonx Orchestrate accelerator for Bob starts a project with a brief of this form:
...
A good prompt is a use case brief, not a question. A complete brief answers six questions. For any question that the brief leaves unanswered, Bob makes an assumption. For an Orchestrate agent, the six questions are:
...
Use a brief to start an agent project, in Ask mode. A brief that answers the six questions receives few questions back. A brief that answers two or three, like the first prompt of chapter 4, receives the others as questions from Bob, which is appropriate when the requirements are not yet settled.

Limitations: the result depends on the completeness of the brief. Bob does not report the assumptions it makes. The closing sentence depends on the mode. In Agent mode, the closing sentence is the only thing that prevents Bob from building immediately. In Ask mode, the mode prevents building and the Ask-mode rules already request questions rather than a design, so the closing sentence only states what you want in the answer.

### Type 3: the structured prompt

A structured prompt has labelled parts, each answering one question that Bob would otherwise have to guess. This guide uses six parts:
...
The labels are a checklist for the writer, so that the expected result and the stopping condition are not forgotten. Bob does not require the labels and understands the same content written as sentences. Omit a part when the rules in the project folder or an approved design already state it. In chapter 4, the Agent-mode prompt is a single line for this reason.
...
A specification prompt requests one file and lists the properties that the file must have. The Planning Analytics workshop for Bob uses this type to write agent definitions:
...
Use a specification prompt for small artifacts whose design is already approved and whose properties are known. The instruction "Return only the YAML" prevents an explanation that you do not need.

Limitations: a specification prompt produces the file and nothing else. Importing and testing require separate prompts. It skips the design conversation, so it is not suitable for a new agent.

### Type 5: the follow-up

A follow-up is a short correction in a conversation that is already in progress. Examples: "Use LL-1003 as the example instead", "Shorter, under 100 words", "You changed the agent's name; put it back".

Use a follow-up to adjust a result that is mostly correct. It is the fastest type, because the context is already in the conversation.

Limitations: a follow-up works while the conversation is short. After many follow-ups, Bob loses track of earlier constraints. Start a new conversation with a complete prompt and references to the current files. Three follow-ups on the same problem indicate that the original prompt was incomplete; rewrite the prompt instead of sending a fourth follow-up.
...
| Start an agent project, explore, get a first proposal | Brief, ending with what you want in the answer | Ask, then Plan |
| Revise a design | Structured, or a follow-up if the change is small | Plan |
| Create or change anything on the instance | Structured, or one line when the design and the rules state the rest | Agent |
| Get one file whose properties you already know | Specification | Plan or Agent |
| Adjust a result that is mostly correct | Follow-up | The current mode |

Every prompt that asks Bob to do something must state two things: the expected result, and the condition under which Bob must stop and ask. The brief states them in its last lines, the structured prompt in the Verify and Stop parts, and the specification prompt in "Return only the file". Most unexpected results come from prompts that omit them.
...
| Requesting a single file (specification) | "Write me an agent YAML for order tracking." | The specification prompt shown in 3.3 | Every property is listed and the schema is named. The weak prompt produces a plausible file with an invented name |
| Adjusting a result (follow-up) | "That's not right, try again." | "The table is right but the answer is too long. Keep the table, remove the introduction, and keep the whole answer under 80 words." | "Try again" gives Bob nothing to change, so it changes something arbitrary |
...
|---|---|---|
| "Build me a customer service agent" | Bob invents the users, the facts, the tools and the names | The six-question brief, in Ask mode |
