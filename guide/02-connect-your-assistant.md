...
Step 3. Click the watsonx Orchestrate icon to open the Explorer view, and click Initialize Workspace. What happens next, in order:

- The extension checks for an existing setup and, if none, asks for permission to install `uv`, a Python package manager. Accept. It creates a `venv` folder in your project with Python 3.12 and the latest ADK inside it. Nothing is installed system-wide.
...
- In Bob only, it copies IBM's seven Orchestrate skills into `.bob/skills` and pre-loads a message in the chat. Send that message when you are ready; the skills are procedures the assistant follows for larger pieces of work, and chapter 12 uses them.
